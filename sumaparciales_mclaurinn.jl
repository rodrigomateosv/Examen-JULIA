...
#Calcular la suma parcial de orden 100 de la serie de Maclaurin de la función $f(x)=\ln(1+x)$ en x equals 1 divided by 2 con 10 decimales.

using SymPy
@vars x real=true
# la función serie devuelve la serie de Taylor alrededor de x=0
# usamos removeO() para eliminar el término "O(x^n)"
p(n) = series(log(1+x), x, 0, n+1).removeO()

# queremos la suma parcial de orden 100
p100 = p(100)
# ahora evaluamos la serie en x = 1/2
val = subs(p100, x => 1/2)
# redondeamos al decimal 10
val = N(val, 10)
println("El valor de la suma parcial de orden 100 en x = 1/2 es ", val)
